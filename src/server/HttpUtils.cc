...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  // TODO: implment
  const char *test_path = test_file.c_str();
  char test [PATH_MAX];
  string test_absolute = realpath(test_path, test);
  
  const char *root_path = root_dir.c_str();
  char root [PATH_MAX];
  string root_absolute = realpath(root_path, root);

  vector<string> root_directories;
  boost::split(root_directories,  root_absolute, boost::is_any_of("/"), boost::token_compress_on);
  vector<string> test_directories;
  boost::split(test_directories,  test_absolute, boost::is_any_of("/"), boost::token_compress_on);

  if(root_directories.size() > test_directories.size()) {return false;}
  for(size_t i = 0; i < root_directories.size(); i++){
    if(root_directories[i] != test_directories[i]) {return false; }
  }
  return true;
}
...
string escape_html(const string &from) {
  ...
  // looked up online.
    ...
    map<char, string> transformations;
    ...
    for(size_t i = 0; i < from.size(); i ++){
      char c = from[i];
      if(transformations.count(c) != 0){
        ret.replace(j, 1, transformations.at(c));
        j += transformations.at(c).length();
      }
      else{j ++;}
    }
  return ret;
